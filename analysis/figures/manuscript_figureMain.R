...
# Filter data, add CKD group var ------------------------------------------------------------------
print("Filter data")

##this is fixed later in the label stage, but the ckd_hist label is no history, and gen is ckd history
df <- df %>% mutate(ckd = str_extract(name, "ckd_hist"))
df$ckd[is.na(df$ckd)] <- "gen"


df <- df[df$model=="mdl_max_adj" & grepl("days",df$term),
         c("cohort","analysis","outcome","ckd", "outcome_time_median","term","hr","conf_low","conf_high")]
...
df <- df <- merge(df, plot_labels[,c("term","label")], by.x = "ckd", by.y = "term", all.x = TRUE)
df <- dplyr::rename(df, "ckd_label" = "label")
...
##I removed analysis_group as the iterator here because for some reason or other, it only gave 'main' and not the sub_covid groups.
#I think I may need another loop for each CKD group, but I'm just trying to get it to work in general, then I can add it.

for (i in unique(df$analysis)){
