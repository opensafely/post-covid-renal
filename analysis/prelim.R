...
prelim_data <- prelim_data %>%
  ...
  mutate(index_vax = max(c(vax_date_covid_2_offset, delta_date), na.rm=T),
         ...
         end_vax = min(c(death_date, deregistration_date), na.rm=T),
         end_unvax_outcome = min(c(death_date, deregistration_date), na.rm=T),
         end_unvax_exposure = min(c(death_date, deregistration_date, vax_date_covid_1), na.rm=T),
         end_prevax_outcome = min(c(death_date, deregistration_date), na.rm=T),
